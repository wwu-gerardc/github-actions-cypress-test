      ...
      if ((typeof context == "undefined" || typeof context != "undefined" && context == document) && typeof window !== "undefined") {
        ...
        class WWUCountdown extends HTMLElement {
          ...
          connectedCallback() {
            ...
            let second = 1e3, minute = second * 60, hour = minute * 60, day = hour * 24;
            ...
            function set_days() {
              days.innerText = d_left;
            }
            function set_hours() {
              hours.innerText = h_left;
            }
            function set_minutes() {
              minutes.innerText = m_left;
            }
            function set_seconds() {
              seconds.innerText = s_left;
            }
            ...
            function second_announce() {
              announcement.innerText = String(s_left);
            }
            function initialize_count(date) {
              ...
              if (d_left > 0 || h_left > 0) {
                set_hours();
              } else {
                hours.parentElement.remove();
              }
            ...
            function update_time(date) {
              ...
              time_left(date);
              if (h_left >= 0 && prev_hours != h_left) {
                set_hours();
              }
              if (m_left >= 0 && prev_minutes != m_left) {
                set_minutes();
              }
              if (s_left >= 0) {
                set_seconds();
              ...
              // Only announce last 10 minutes, and only when the minute is about to change
                            if (d_left <= 0 && h_left <= 0 && m_left <= 10 && m_left >= 1 && s_left == 0) {
                minute_announce();
              }
              // Only announce last 10 seconds
                            if (d_left <= 0 && h_left <= 0 && m_left <= 0 && s_left <= 10 && s_left > 0) {
                second_announce();
              }
            }
          }
